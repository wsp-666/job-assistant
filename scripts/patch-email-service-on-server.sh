#!/bin/bash
# 在云端执行：bash scripts/patch-email-service-on-server.sh
# 或上传本脚本后：cd /www/wwwroot/job-assistant && bash scripts/patch-email-service-on-server.sh

set -euo pipefail
TARGET="/www/wwwroot/job-assistant/api/app/services/email_service.py"
...
echo "已写入: $TARGET"
grep -n "_build_from_header\|server.sendmail" "$TARGET" | head -5
echo "请在宝塔重启 Python 项目后重新测试发信。"
